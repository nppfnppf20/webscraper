<script>
  ...
  async function refreshRenewablesTest2() {
    const response = await fetch('http://127.0.0.1:8000/api/refresh/planit-test2', { method: 'POST' });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    return await response.json();
  }
  ...
  onMount(async () => {
    try {
      ...
      error = e?.message || 'Failed to load renewables data';
    } finally {
      loading = false;
    }
  });
  ...
  async function refreshNow() {
    try {
      ...
      const result = await refreshRenewablesTest2();
...
{#if loading}
  <div class="text-center p-4">
    ...
    <p class="mt-2">Loading renewables data…</p>
  </div>
{:else if error}
  <div class="alert alert-danger">{error}</div>
{:else}
  <div class="results-summary">
    <p><strong>{renewables.length}</strong> renewables projects found</p>
    {#if renewables.length > 0}
      {@const statusCounts = renewables.reduce((acc, r) => { acc[r.app_state || 'Unknown'] = (acc[r.app_state || 'Unknown'] || 0) + 1; return acc; }, {})}
      {@const authorityCounts = renewables.reduce((acc, r) => { acc[r.area_name || 'Unknown'] = (acc[r.area_name || 'Unknown'] || 0) + 1; return acc; }, {})}
  ...
  <DataTable
    ...
    {loading}
    {error}
    searchPlaceholder="Search renewables projects..."
    emptyMessage="No renewables projects found for the last 30 days. Try refreshing the data."
    showSearch={true}
    showActions={false}
    minWidth="1400px"
  />
